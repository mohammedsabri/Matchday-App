...
class DatabaseHelper(context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("CREATE TABLE $TABLE_NAME (ID INTEGER PRIMARY KEY " +
                "AUTOINCREMENT,NAME TEXT,POSITION TEXT,TEAM TEXT)")
    }
     ...
     */
    fun insertData(name: String, POSITION: String, team: String) {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        contentValues.put(COL_2, name)
        contentValues.put(COL_3, POSITION)
        contentValues.put(COL_4, team)
     ...
     */
    fun updateData(id: String, name: String, position: String, team: String):
            Boolean {
        val db = this.writableDatabase
        val contentValues = ContentValues()
